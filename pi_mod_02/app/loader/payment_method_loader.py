...
from app.loader.constants import SQL_TO_PAYMENT_METHOD_MODEL
...
class PaymentMethodLoader(DataLoader):
    priority = LoaderTablesPriority.PAYMENT_METHODS
    sql_to_model = SQL_TO_PAYMENT_METHOD_MODEL
    file_path = LoaderSQLFilePaths.PAYMENT_METHODS
    table_name = TableNames.PAYMENT_METHODS

    # TODO refactoizar para no repetir
    def _parse_file(self, content: str) -> list[dict]:
        # Eliminar "USE ..." y "GO"
        content = re.sub(r"(?i)USE\s+\w+;", "", content)
        content = re.sub(r"(?i)\bGO\b", "", content)

        pattern = r"INSERT INTO MetodosPago\s*\((.*?)\)\s*VALUES\s*(.*?);"
    ...
    def _save_records(self, db, records):
        payments_records = [PaymentMethod.from_dict(rec) for rec in records]
        print(
            f"Guardando {len(payments_records)} metodos de pago en la base de datos..."
        )
        try:
            db.bulk_save_objects(payments_records)
            db.commit()
            print(
                f"{len(payments_records)} metodos de pago creados correctamente de un total de {len(records)}."
            )
